#!/usr/bin/env python3
"""Train + eval success for the tableside table-spawn touch mix50 run.

Train: raw ``train_success_1000``.
Eval: faint raw + bold rolling mean (window=5).

  python scripts/plot_allegro_tableside_tablespawn_touch_success.py
"""
...
OUT = os.path.join(
    ...
    'akt_tableside_tablespawn_grasp_train_eval_success.png')
RUNS = (
    (
        ...
        'table-spawn grasp + mix50 + extrew1 (no AR)',
        2,
    ),
)
...
def main() -> None:
  ...
  axes[0].set_title('Allegro tableside table-spawn grasp — train (raw)',
