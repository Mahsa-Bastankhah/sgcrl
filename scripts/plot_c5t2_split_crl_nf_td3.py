#!/usr/bin/env python3
"""One-off: split creative5_task2 train/eval success into CRL / NF / TD3.

Only catselect runs (the previous combined overlay + any other catselect
dirs that still have learner data). No InfoNCE runs exist for this task.
Does not touch the general plotter.
"""
...
RUNS_BY_FAMILY: dict[str, list[str]] = {
    'crl': [
        'ppo_builderbench_creative5_task2_e1024_pd_crl_tau05_actorreset_evalvid_catselect_extrew1',
        'ppo_builderbench_creative5_task2_e1024_pd_crl_tau05_actorreset_nopermute_normobs_evalvid_catselect',
        'ppo_builderbench_creative5_task2_e1024_pd_crl_tau05_actorreset_nopermute_normobs_evalvid_catselect_extrew1',
    ],
    'nf': [
    ...
    'td3': [
        'ppo_builderbench_creative5_task2_e1024_pd_td3_logq_tau05_actorreset_evalvid_catselect_extrew1',
        'ppo_builderbench_creative5_task2_e1024_pd_td3_logq_tau05_actorreset_nopermute_normobs_evalvid_catselect',
        'ppo_builderbench_creative5_task2_e1024_pd_td3_logq_tau05_actorreset_nopermute_normobs_evalvid_catselect_extrew1',
    ],
}
...
def _family_of(name: str) -> str | None:
  low = name.lower()
  if '_tdinfonce_' in low or '_infonce_' in low:
    return 'crl'
...
def _plot(family: str, log_dirs: list[str], *, split: str) -> str | None:
  ...
  ax.set_title(
      f'BuilderBench Creative 5 Task 2 [{FAMILY_TITLE[family]}] — '
      f'{title_metric}',
...
def main():
  ...
  for name in sorted(os.listdir(LOG_ROOT)):
    if not name.startswith('ppo_builderbench_creative5_task2'):
      continue
    if 'catselect' not in name or 'prenorm' in name:
      continue
    # Skip job-id-suffixed duplicates of the same config.
    if re.search(r'_\d{7,}$', name):
      continue
