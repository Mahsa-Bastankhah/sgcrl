#!/usr/bin/env python3
"""Finger-gun NF screen (compact-small, no grad-reg).

NF-only until this pose is solved.

  python scripts/plot_allegro_hand16gun_nf.py
  python scripts/plot_allegro_hand16gun_nf.py --watch 30 --watch-max-sec 3600
"""
...
NF_LOG_DIR = (
    'ppo_allegro_hand16gun_e1024_nf_compactsmall_fully_scaled_ep50_200m_seed0_4h'
)
MPO_LOG_DIR = 'mpo_crl_allegro_hand16gun_e1024_ep50_200m_seed0_4h'
OUT = os.path.join(
    REPO, 'figs', 'allegro_kuka_throw', 'hand16gun_nf_success.png')
...
def run_once(*, out: str = OUT) -> str:
  ...
  if not nfx_ev and not nfx_tr:
    ax.text(
        0.02, 0.92,
        'NF: no series yet (job crashed at PhysX init; resubmitted)',
  ...
  if nfx_tr:
    ...
    last_m = nfx_ev[-1] / 1e6
  ax.set_title(
      f'Gun · NF + MPO · eval roll mean w={w} · '
      f'@ {last_m:.1f}M',
...
def main() -> None:
  ...
  t0 = time.time()
  print(
      f'[hand16gun] watching every {args.watch}s '
  ...
  while True:
    try:
      ...
      print(f'[hand16gun] error: {exc}', flush=True)
    if time.time() - t0 >= args.watch_max_sec:
      print('[hand16gun] watch-max-sec reached; exiting', flush=True)
      return
