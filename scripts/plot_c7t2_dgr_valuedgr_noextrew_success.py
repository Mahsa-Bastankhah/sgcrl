#!/usr/bin/env python3
"""c7t2 NF+value dual-dgr, no external reward: per-seed success and ‖∇_s V‖.

Train raw. Eval faint raw + roll mean w=5. Value-grad from learner CSV.

  python scripts/plot_c7t2_dgr_valuedgr_noextrew_success.py
"""
...
OUT = os.path.join(
    ...
    'c7t2_dgr_valuedgr_noextrew_per_seed_success_grad_s.png')
C = base.ACCENT_COLORS
GRAD_REG_C = 100.0
NAME = (
    'ppo_builderbench_creative7_task2_e1024_pd'
    '_nf_compact_small_sa3x192_r64_b6_w192_tau05_'
    'nopermute_fixedx01_catwp_noextrew_minstd1e5_ent05to001_'
...
def main() -> None:
  ...
  for ax, title, ylabel, ylim in (
      (axes[0], 'c7t2  ·  NF+value dgr  ·  no extrew  ·  train by seed',
       'Train Success (last 1000)', (-0.05, 1.05)),
      (axes[1],
       f'c7t2  ·  NF+value dgr  ·  no extrew  ·  eval by seed (roll mean w={w})',
       f'Eval Success (roll mean w={w})', (-0.05, 1.05)),
      (axes[2], r'c7t2  ·  no extrew  ·  $\|\nabla_s V\|$  by seed',
       r'$\|\nabla_s V\|$', None),
  ):
  ...
  fig.text(
      ...
      r'NF + value dualgradreg $c=100$ $\lambda$lr$=10^{-6}$. No external reward.',
