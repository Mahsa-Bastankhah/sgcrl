#!/usr/bin/env python3
"""c7t2 NF dual-dgr + value dual-dgr: per-seed success and ‖∇_s V‖.

Train raw. Eval faint raw + roll mean w=5. Value-grad from learner CSV.

  python scripts/plot_c7t2_dgr_valuedgr_success.py
"""
...
OUT = os.path.join(
    ...
    'c7t2_dgr_valuedgr_per_seed_success_grad_s.png')
...
def main() -> None:
  ...
  for ax, title, ylabel, ylim in (
      (axes[0], 'c7t2  ·  NF dgr + value dgr  ·  train by seed',
       'Train Success (last 1000)', (-0.05, 1.05)),
      (axes[1],
       f'c7t2  ·  NF dgr + value dgr  ·  eval by seed (roll mean w={w})',
