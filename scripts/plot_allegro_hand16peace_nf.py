#!/usr/bin/env python3
"""Peace / V-sign NF screen (compact-small, no grad-reg).

NF-only until RND/MPO are launched on this pose.

Eval: faint raw + bold rolling mean, window=5.
Train: raw.

  python scripts/plot_allegro_hand16peace_nf.py
  python scripts/plot_allegro_hand16peace_nf.py --watch 30 --watch-max-sec 3600
"""
...
NF_LOG_DIR = (
    'ppo_allegro_hand16peace_e1024_nf_compactsmall_fully_scaled_ep50_200m_seed0_4h'
)
MPO_LOG_DIR = 'mpo_crl_allegro_hand16peace_e1024_ep50_200m_seed0_4h'
OUT = os.path.join(
    REPO, 'figs', 'allegro_kuka_throw', 'hand16peace_nf_success.png')
...
def run_once(*, out: str = OUT) -> str:
  ...
  if nfx_tr:
    ...
    last_m = nfx_ev[-1] / 1e6
  ax.set_title(
      f'Peace / V-sign · NF + MPO · eval roll mean w={w} · '
      f'@ {last_m:.1f}M',
...
def main() -> None:
  ...
  t0 = time.time()
  print(
      f'[hand16peace] watching every {args.watch}s '
  ...
  while True:
    try:
      ...
      print(f'[hand16peace] error: {exc}', flush=True)
    if time.time() - t0 >= args.watch_max_sec:
      print('[hand16peace] watch-max-sec reached; exiting', flush=True)
      return
