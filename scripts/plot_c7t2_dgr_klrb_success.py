#!/usr/bin/env python3
"""c7t2 blue dgr + skip PPO after 4× train_success_1000 > 0.8.

Train: raw train_success_1000. Eval: faint raw + roll mean w=5.

  python scripts/plot_c7t2_dgr_klrb_success.py
"""
...
OUT = os.path.join(
    ...
    'c7t2_dgr_trainsucc08x4_skipppo_train_eval_success.png')
OUT_SEEDS = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    'figs', 'builderbench', 'final_runs',
    'c7t2_dgr_trainsucc08x4_skipppo_per_seed_success.png')
...
NAME = (
    ...
    'ep70_300m_crl10_dualgradreg_c100_lamlr1e6_'
...
def _plot_per_seed() -> None:
  ...
  _style(
      axes[0],
      title='c7t2  ·  dgr + train-latch skip PPO  ·  train by seed',
      ylabel='Train Success (last 1000)', xlabel=False)
  _style(
      axes[1],
      title=f'c7t2  ·  dgr + train-latch skip PPO  ·  eval by seed (roll mean w={w})',
...
def main() -> None:
  ...
  _style(
      axes[0],
      title='c7t2  ·  dgr + train-latch skip PPO  ·  train',
      ylabel='Train Success (last 1000)', xlabel=False)
  _style(
      axes[1],
      title=f'c7t2  ·  dgr + train-latch skip PPO  ·  eval (roll mean w={w})',
      ylabel=f'Eval Success (roll mean w={w})', xlabel=True)
  axes[0].legend(
      loc='upper left', fontsize=8.5, framealpha=0.95,
      fancybox=False, edgecolor='#333333')
  fig.text(
      0.5, 0.015,
      'Same compact-small dgr as the c7t2 no-tKL dual-dgr run (T=ep=70). After 4 consecutive '
