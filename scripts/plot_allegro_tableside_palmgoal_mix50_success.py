#!/usr/bin/env python3
"""Train + eval success for the tableside palm-on-top mix50 run.

Train: raw ``train_success_1000``.
Eval: faint raw + bold rolling mean (window=5).

  python scripts/plot_allegro_tableside_palmgoal_mix50_success.py
"""
...
OUT = os.path.join(
    ...
    'akt_tableside_palmgoal_mix50_train_eval_success.png')
RUNS = (
    (
        ...
        'initrand + palm-on-top + mix50 + AR + extrew1',
        1,
    ),
)
...
def main() -> None:
  ...
  axes[0].set_title('Allegro tableside palm-on-top — train (raw)',
