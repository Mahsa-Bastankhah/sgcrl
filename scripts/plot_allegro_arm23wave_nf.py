#!/usr/bin/env python3
"""Wave / 23-D arm+hand NF screen (compact-small, no grad-reg).
...
Train: raw.
  ...
  python scripts/plot_allegro_arm23wave_nf.py
  python scripts/plot_allegro_arm23wave_nf.py --watch 30 --watch-max-sec 3600
"""
...
NF_LOG_DIR = (
    'ppo_allegro_arm23wave_e1024_nf_compactsmall_fully_scaled_ep50_200m_seed0_4h'
)
MPO_LOG_DIR = 'mpo_crl_allegro_arm23wave_e1024_ep50_200m_seed0_4h'
OUT = os.path.join(
    REPO, 'figs', 'allegro_kuka_throw', 'arm23wave_nf_success.png')
...
def run_once(*, out: str = OUT) -> str:
  ...
  if nfx_tr:
    ...
    last_m = nfx_ev[-1] / 1e6
  ax.set_title(
      f'Wave 23-D · NF + MPO · eval roll mean w={w} · '
      f'@ {last_m:.1f}M',
...
def main() -> None:
  ...
  t0 = time.time()
  print(
      f'[arm23wave] watching every {args.watch}s '
  ...
  while True:
    try:
      ...
      print(f'[arm23wave] error: {exc}', flush=True)
    if time.time() - t0 >= args.watch_max_sec:
      print('[arm23wave] watch-max-sec reached; exiting', flush=True)
      return
