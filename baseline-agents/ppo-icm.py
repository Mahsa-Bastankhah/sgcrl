...
class Args:
    ...
    # Pathak et al. ICML 2017 ICM (continuous-action adaptation).
    # Loss = (1 - icm_beta) * L_inv + icm_beta * L_fwd; paper / official beta=0.2.
    icm_beta: float = 0.2
    icm_feature_dim: int = 256
    # Residual ICM features are unbounded; L2-normalize φ and clip the bonus
    # (Pathak official REWARD_CLIP=1) so returns/losses cannot explode to NaN.
    icm_l2_normalize_features: bool = True
    icm_reward_clip: float = 1.0
...
def make_ppo_networks(args, action_size, include_auxiliary=True):
    """Build PPO+ICM modules with architecture inferred from Args."""
...
class ICM(nn.Module):
    """Pathak et al. ICM: inverse + forward models in a learned feature space."""
    ...
    def setup(self):
        if self.use_residual_mlp:
            hidden = list(_RESIDUAL_HIDDEN_SIZES)
            self.encoder = ResidualMLP(
                layer_sizes=hidden + [self.feature_dim],
                activation=self.activation)
            self.inverse_model = ResidualMLP(
                layer_sizes=hidden + [self.action_size],
                activation=self.activation)
            self.forward_model = ResidualMLP(
                layer_sizes=hidden + [self.feature_dim],
                activation=self.activation)
        else:
            self.encoder = MLP(
                layer_sizes=[self.hidden_size, self.feature_dim],
                activation=self.activation)
            self.inverse_model = MLP(
                layer_sizes=[self.hidden_size, self.action_size],
                activation=self.activation)
            self.forward_model = MLP(
                layer_sizes=[self.hidden_size, self.feature_dim],
                activation=self.activation)

    def _normalize(self, x, normalizer_params):
        if normalizer_params is not None:
            x = (x - normalizer_params.mean) / (normalizer_params.std)
            x = jnp.clip(x, -5, 5)
        return x
    ...
    def __call__(self, obs, next_obs, action, normalizer_params=None,
                 l2_normalize_features=True):
        obs = self._normalize(obs, normalizer_params)
        next_obs = self._normalize(next_obs, normalizer_params)
        phi_s = self.encoder(obs)
        phi_next = self.encoder(next_obs)
        if l2_normalize_features:
            phi_s = self._l2_normalize(phi_s)
            phi_next = self._l2_normalize(phi_next)
        pred_action = self.inverse_model(
            jnp.concatenate([phi_s, phi_next], axis=-1))
        # Official: no backprop into the sampled action (it is a constant here).
        pred_phi_next = self.forward_model(
            jnp.concatenate([phi_s, jax.lax.stop_gradient(action)], axis=-1))
        if l2_normalize_features:
            pred_phi_next = self._l2_normalize(pred_phi_next)
        return pred_phi_next, phi_next, pred_action
...
def main(args: Args):
    ...
    local_key, key_env, key_eval, key_policy, key_value, key_icm = jax.random.split(key, 6)
    ...
    if use_allegro:
        ...
        print(f"ICM beta={args.icm_beta} feature_dim={args.icm_feature_dim} "
              f"use_residual_mlp={args.use_residual_mlp} "
              f"l2_normalize_features={args.icm_l2_normalize_features} "
              f"reward_clip={args.icm_reward_clip} "
              f"(mixed hard-success + curiosity)")
        if args.use_residual_mlp:
            print("ICM encoder/inverse/forward: ResidualMLP 6x256 LN+swish, "
                  "skip every 2 (same trunk as actor/value)")
    ...
    training_state = PPOTrainingState.create(
        ...
        params={
            ...
            'icm': ppo_network.icm_network.init(
                key_icm,
                jnp.zeros((1, obs_size + goal_size)),
                jnp.zeros((1, obs_size + goal_size)),
    ...
    def data_collect_step(training_state, env_state, key_generate_rollout):
        ...
        # Updating collected data with intrinsic rewards (forward prediction error)
        pred_phi, phi_next, _ = ppo_network.icm_network.apply(
            training_state.params['icm'],
            data.observation,
            data.next_observation,
            data.action,
            training_state.normalizer_params,
            l2_normalize_features=args.icm_l2_normalize_features,
        )
        int_rewards = jnp.clip(
            jnp.sum((pred_phi - phi_next) ** 2, axis=-1) / 2,
            0.0, args.icm_reward_clip)
        data.extras['policy_extras']['int_reward'] = int_rewards
        data_metrics = {**data_metrics, 'int_reward': jnp.mean(int_rewards)}
    ...
    @jax.jit
    def attach_icm_rewards(training_state, data):
        pred_phi, phi_next, _ = ppo_network.icm_network.apply(
            training_state.params['icm'],
            data.observation,
            data.next_observation,
    ...
    def allegro_data_collect_step(training_state, packed, key_generate_rollout):
        ...
        training_state, data = attach_icm_rewards(training_state, data)
    ...
    def compute_ppo_loss(
        ...
        icm_apply = ppo_network.icm_network.apply
        ...
        # ICM: inverse (action from φ(s),φ(s')) + forward (φ(s') from φ(s),a)
        pred_phi, phi_next, pred_action = icm_apply(
            params['icm'],
            data.observation,
            data.next_observation,
            data.action,
        ...
        forward_loss = jnp.mean((pred_phi - phi_next) ** 2)
        inverse_loss = jnp.mean((pred_action - data.action) ** 2)
        icm_loss = (
            (1.0 - args.icm_beta) * inverse_loss
            + args.icm_beta * forward_loss
        )
        ...
        total_loss = policy_loss + v_loss + int_v_loss + entropy_loss + icm_loss
        return total_loss, {
